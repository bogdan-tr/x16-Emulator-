...
uint16_t getimmediate(uint16_t instruction) { return getbit(instruction, 5); }
...
uint16_t emit_jmp(reg_t base) { return (OP_JMP << 12) | (base << 6); }
...
uint16_t emit_jsrr(reg_t reg) { return (OP_JSR << 12) | (reg << 6); }
...
uint16_t emit_trap(trap_t vec) { return (OP_TRAP << 12) | (vec & 0xff); }

// Emit a value
uint16_t emit_value(uint16_t val) { return val; }
